...
module usbdev_reg_top #(
        ...
        always_comb begin
                ...
                if ((bus_addr_i >= USBDEV_BUFFER_OFFSET) && (bus_addr_i < (USBDEV_BUFFER_OFFSET + (SramDepth * 4)))) begin
                        reg_steer = 1'b0;
                end else begin
                        reg_steer = 1'b1;
                end
        ...
        assign reg_we      = bus_we_i & reg_steer;
        assign reg_re      = bus_re_i & reg_steer;
        assign reg_addr    = bus_addr_i;
        assign reg_wdata   = bus_wdata_i;
        assign reg_be      = bus_be_i;
        assign bus_rdata_o = reg_rdata;
        assign bus_error_o = reg_error;

        //get bus data if writing to or reading from sram
        assign sram_we     = bus_we_i & ~reg_steer;
        assign sram_re     = bus_re_i & ~reg_steer;

        //assign to sram
        assign ram_req_o   = sram_we | sram_re;
        assign ram_we_o    = bus_we_i;
        assign ram_addr_o  = bus_addr_i[SramAw+1:2];  // [10:2] for SramAw=9
        assign ram_wdata_o = bus_wdata_i;
