...
sim: ## Run RTL simulation with cocotb
	cd cocotb; PDK_ROOT=${PDK_ROOT} PDK=${PDK} SLOT=${SLOT} PAD=${PAD} SCL=${SCL} SRAM=${SRAM}  pytest -nauto --maxschedchunk=1 --dist worksteal -v chip_top_tb.py
.PHONY: sim

sim-gl: ## Run gate-level simulation with cocotb (after copy-final)
	cd cocotb; GL=1 PDK_ROOT=${PDK_ROOT} PDK=${PDK} SLOT=${SLOT} PAD=${PAD} SCL=${SCL} SRAM=${SRAM}  pytest -nauto --maxschedchunk=1 --dist worksteal -v chip_top_tb.py
.PHONY: sim-gl
...
copy-final: ## Copy final output files from the last run
	...
	md5sum final/gds/chip_top.gds > final/gds/naxos.gds.md5sum
	gzip -fc9 final/gds/chip_top.gds > final/gds/naxos.gds.gz
.PHONY: copy-final
